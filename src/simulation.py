...
class Simulation:
    ...
    def run_gen(self):
        ...
        with progressbar.ProgressBar(max_value=self.sim_steps) as bar:
            for n in range(self.sim_steps):
                ...
                yield self.positions, self.last_angle


        self.sim_results = self.positions
        self.total_time = tottime
        return self.sim_results, self.last_angle
    ...
    def get_particle_interaction(self, k, all_interactions=False):
        ...
        if all_interactions:
            all_positions = handler.positions
            for other_idx in range(len(all_positions)):
                if other_idx == k:
                    continue
                nb_pos = self.positions[other_idx]
